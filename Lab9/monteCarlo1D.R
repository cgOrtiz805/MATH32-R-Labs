...
integrand <- function(x) {1/(x^2+9)} #<-- Will change based on function
...
plot(x, integrand(x),type = 'l',
     col='black',main="f(x) = 1/(x^2+9)",ylab="f(x)")

################################
# (1) Calculus (i.e., By Hand) #
################################

antiDerivative <- function(x) {(1/3)*atan(x/3) } #<--Will Need to Change
